<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create New Issue for ' . $project->name) }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <h1 class="text-3xl font-bold text-gray-800 mb-6">Create New Issue for {{ $project->name }}</h1>

                    <div class="bg-white shadow-md rounded-lg p-6">
                        <form action="{{ route('projects.issues.store', $project) }}" method="POST">
                            @csrf
                            <input type="hidden" name="project_id" value="{{ $project->id }}">
                            @include('issues.form', ['issue' => new \App\Models\Issue()])
